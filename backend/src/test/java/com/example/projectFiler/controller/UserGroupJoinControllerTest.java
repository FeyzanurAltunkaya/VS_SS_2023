...
public class UserGroupJoinControllerTest {
    ...
    public void testAddUserToGroup() throws Exception {
        ...
        mockMvc.perform(post("/usergroup/{userId}/groups/{groupId}", user.getId(), group.getId()))
        ...
        verify(userGroupJoinRepository, times(1)).save(ArgumentMatchers.any(UserGroupJoinEntity.class));
    }
    ...
    public void testRemoveUserFromGroup() throws Exception {
        ...
        mockMvc.perform(delete("/usergroup/{userId}/groups/{groupId}", 1L, 1L))
                .andExpect(status().isNoContent());

        // Verify the interactions
        verify(userRepository, times(1)).findById(1L);
        verify(groupRepository, times(1)).findById(1L);
        verify(userGroupJoinRepository, times(1)).deleteByUserAndGroup(userEntity, groupEntity);
    }
}
